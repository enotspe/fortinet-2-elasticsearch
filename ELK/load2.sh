...
if [ "$AUTH_METHOD" != "user" ] && [ "$AUTH_METHOD" != "apikey" ]; then
  echo "Error: AUTH_METHOD must be 'user' or 'apikey'"
  exit 1
fi
...
if [ "$TEST_CREDENTIALS" == "true" ]; then
  ...
  echo "  - Fortinet templates: $LOAD_FORTINET"
...
if [ "$LOAD_FORTINET" == "true" ]; then
  echo -e "\nLoading custom Fortinet component templates..."
  upload_fortinet_templates
fi
